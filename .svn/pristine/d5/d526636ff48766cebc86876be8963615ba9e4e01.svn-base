...
public class FindTransLogByTimeAction extends ActionSupport{
    ...
    /**
     * 调用TransFacade.findTransLogByTime()
     */
    ...
    protected Object processBody(HashMap hashBody) throws ManageInputException {
        String beginTime = TypeCast.stringToString((String) hashBody.get("beginTime"),"开始时间",false);
        String endTime = TypeCast.stringToString((String) hashBody.get("endTime"),"结束时间",false);
        ...
        try{
            ...
            throw new ManageInputException("开始时间格式错误");
        }
        try{
            endTime = DateUtil.converToString(endTime,2);
        }catch(Exception ex){
            throw new ManageInputException("结束时间格式错误");
        }
