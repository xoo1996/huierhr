...
/* 	var myDate = new Date();
	...
	$("input[name=departmentid]").autocomplete(shops,{
		max : 10,
		matchContains : true
	});
	$("input[name=departmentid]").result(function(event, data, formatted) {
		if (data){
			var gnm = data[0].substring(data[0].indexOf("=")+1);
			var gid = data[0].substring(0,data[0].indexOf("="));
			$("input[name=departmentid]").val(gid);
...
<lemis:body>
	...
	<COLGROUP><COL width='10%'><COL width='15%'><COL width='10%'><COL width='15%'><COL width='10%'><COL width='15%'><COL width='10%'><COL width='15%'></COLGROUP>
		<html:form action="/ApaOperAction.do?method=saveModifiedApa&type=user" method="POST">
			<tr>
				<lemis:texteditor property="nemid" label="流程id" disable="true" 
					required="true" />
				<lemis:texteditor property="useremployid" label="工号"
					required="true" disable="true" maxlength="10" />
			...
			<tr>
					...
					<lemis:formateditor mask="date" property="userworktime" format="true"
						label="参加工作时间"  required="false" disable="false"/>
					...
					<lemis:formateditor property="usercondateend" mask="date"
						label="合同到期日期" format="true" disable="false" required="false" />
			</tr>
			<tr>
			...
			<table class="tableinput">
				...
				<tr>
					...
					<lemis:texteditor property="userpostcode1" label="邮编"
						disable="false" required="false" maxlength="6"/>
				<lemis:codelisteditor type="usercontype" label="合同类型" required="false" isSelect="true" redisplay="true"/>
				</tr>			
			</table>
