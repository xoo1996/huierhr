<!--/lemis/recommendation/employassembly/QueryEmployInvite.jsp-->
...
String menuId = (String)request.getSession().getAttribute("menuId");
 ...
 header.add(new Formatter("acb202","招聘地点"));
 ...
 header.add(new Formatter("aae005","联系电话"));
  ...
  List buttons = new ArrayList();
  	...
  	buttons.add(new Button("Btn_edit", "修改招聘信息", "rec070202", "edit(document.all.tableform)"));
  	buttons.add(new Button("Btn_next", "修改岗位信息", "rec070203", "next(document.all.tableform)"));
...
%>
  ...
  <lemis:query action="/Rec0702Action.do?method=queryEmployInvite"  editorMeta="editor" topic="查询条件"/>

  <lemis:table topic="单位招聘信息" action="/Rec0702Action.do" headerMeta="header" hiddenMeta="hidden" 
     mode="radio"/>
...
<script language="javascript" >
  ...
  function view(obj)
  {
    ...
    {return t;
      ...
      obj.action='<html:rewrite page="/Rec0702Action.do?method=viewEmployInvite&"/>'+getAlldata(obj);
  ...
  function edit(obj){
   ...
   obj.action= '<html:rewrite page="/Rec0702Action.do?method=modEmployInvite&"/>'+getAlldata(obj);
...
function next (obj){
    ...
    obj.action= '<html:rewrite page="/Rec0702Action.do?method=toNext&"/>'+getAlldata(obj);
