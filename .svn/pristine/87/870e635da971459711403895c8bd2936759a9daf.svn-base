/**
 * Rec0702DTO.java 2008/03/27
 * 
 * Copyright (c) 2008 - 2017
 * All rights reserved.<br>
 * @author lwd
 * @version 1.0
 */
package org.radf.apps.recommendation.employassembly.dto;

import java.math.BigDecimal;
import java.util.Date;

import org.radf.plat.util.entity.EntitySupport;

/**
 * 大会招聘dto
 */
public class Rec0702DTO extends EntitySupport {

	// ab01
	private String aab001; // 单位编号

	private String aab003; // 组织机构代码

	private String aab019; // 单位类型

	private String aab020; // 经济类型

	private String aab022; // 行业类型

	private String aab004; // 单位名称

	private String aab002;// 社会保险登记证编码

	private String aab007;// 工商登记执照号码

	private String aab043;// 单位拼音码

	// CB20
	private String aac002;// 公民身份号码

	private String aae004;// 联系人

	private String aae005;// 联系电话

	private Date aae043;// 登记日期

	private String acb201;// 招聘方式

	private String acb202;// 招聘地点

	private String acb203;// 招聘地区

	private String acb204;// 工作地点

	private String acb205;// 乘车路线

	private String acb206;// 发布名称

	private Date acb207;// 面试日期

	private BigDecimal acb209;// 收费金额

	private String acb20b;// 招聘信息采集员

	private String acb20c;// 面试地点

	private String acb20f;// 面试详细说明

	private String bcb991;// 职业介绍类型

	// CB21

	private String aac004;// 性别要求

	private String aac009;// 户口性质

	private String aac011;// 文化程度

	private String aac048;// 用工形式

	private String aac014;// 专业技术职务

	private String aac015;// 国家职业资格等级（工人技术等级）

	private String aac017;// 婚姻状况

	private String aac024;// 政治面貌

	private BigDecimal aac034;// 身高

	private String aac036;// 视力

	private String aae011;// 经办人

	private String aae013;// 备注

	private String aae017;// 经办机构

	private String aae019;// 经办科室

	private Date aae030;// 开始日期

	private Date aae031;// 终止日期

	private Date aae036;// 经办日期

	private String aae101;// 修改人员

	private Date aae136;// 修改时间

	private String aca111;// 职业（工种）

	private String aca112;// 工种名称

	private String acb035;// 是否招收大龄人员

	private String acb200;// 招聘编号

	private String acb208;// 有效标记

	private String acb20d;// 注销人员

	private Date acb20e;// 注销日期

	private String acb210;// 空位编号

	private Short acb211;// 委托招聘人数

	private String acb214;// 合同期限

	private String acb215;// 招收区域限制

	private String acb216;// 工种说明

	private String acb217;// 招收区域

	private Short acb218;// 已推荐人数

	private Short acb21a;// 已成功人数

	private String acb21c;// 是否招收外来人员

	private Short acb21d;// 允许人数

	private String acb21g;// 社会保险情况

	private BigDecimal acb21h;// 劳动报酬

	private String acb21i;// 工资待遇说明

	private Short acb221;// 最小年龄

	private Short acb222;// 最大年龄

	private String acb228;// 是否提供食宿

	private String bac299;// 求职人员类别

	private String acb230;// 招聘期数编号
	
	private String acb231;// 招聘期数

	public String getAcb231() {
		return acb231;
	}

	public void setAcb231(String acb231) {
		this.acb231 = acb231;
	}

	public String getAcb230() {
		return acb230;
	}

	public void setAcb230(String acb230) {
		this.acb230 = acb230;
	}

	public String getAab001() {
		return aab001;
	}

	public void setAab001(String aab001) {
		this.aab001 = aab001;
	}

	public String getAab003() {
		return aab003;
	}

	public void setAab003(String aab003) {
		this.aab003 = aab003;
	}

	public String getAab004() {
		return aab004;
	}

	public void setAab004(String aab004) {
		this.aab004 = aab004;
	}

	public String getAab019() {
		return aab019;
	}

	public void setAab019(String aab019) {
		this.aab019 = aab019;
	}

	public String getAab020() {
		return aab020;
	}

	public void setAab020(String aab020) {
		this.aab020 = aab020;
	}

	public String getAab022() {
		return aab022;
	}

	public void setAab022(String aab022) {
		this.aab022 = aab022;
	}

	public String getAac002() {
		return aac002;
	}

	public void setAac002(String aac002) {
		this.aac002 = aac002;
	}

	public String getAac004() {
		return aac004;
	}

	public void setAac004(String aac004) {
		this.aac004 = aac004;
	}

	public String getAac009() {
		return aac009;
	}

	public void setAac009(String aac009) {
		this.aac009 = aac009;
	}

	public String getAac011() {
		return aac011;
	}

	public void setAac011(String aac011) {
		this.aac011 = aac011;
	}

	public String getAac048() {
		return aac048;
	}

	public void setAac048(String aac048) {
		this.aac048 = aac048;
	}

	public String getAac014() {
		return aac014;
	}

	public void setAac014(String aac014) {
		this.aac014 = aac014;
	}

	public String getAac015() {
		return aac015;
	}

	public void setAac015(String aac015) {
		this.aac015 = aac015;
	}

	public String getAac017() {
		return aac017;
	}

	public void setAac017(String aac017) {
		this.aac017 = aac017;
	}

	public String getAac024() {
		return aac024;
	}

	public void setAac024(String aac024) {
		this.aac024 = aac024;
	}

	public BigDecimal getAac034() {
		return aac034;
	}

	public void setAac034(BigDecimal aac034) {
		this.aac034 = aac034;
	}

	public String getAac036() {
		return aac036;
	}

	public void setAac036(String aac036) {
		this.aac036 = aac036;
	}

	public String getAae004() {
		return aae004;
	}

	public void setAae004(String aae004) {
		this.aae004 = aae004;
	}

	public String getAae005() {
		return aae005;
	}

	public void setAae005(String aae005) {
		this.aae005 = aae005;
	}

	public String getAae011() {
		return aae011;
	}

	public void setAae011(String aae011) {
		this.aae011 = aae011;
	}

	public String getAae013() {
		return aae013;
	}

	public void setAae013(String aae013) {
		this.aae013 = aae013;
	}

	public String getAae017() {
		return aae017;
	}

	public void setAae017(String aae017) {
		this.aae017 = aae017;
	}

	public String getAae019() {
		return aae019;
	}

	public void setAae019(String aae019) {
		this.aae019 = aae019;
	}

	public Date getAae030() {
		return aae030;
	}

	public void setAae030(Date aae030) {
		this.aae030 = aae030;
	}

	public Date getAae031() {
		return aae031;
	}

	public void setAae031(Date aae031) {
		this.aae031 = aae031;
	}

	public Date getAae036() {
		return aae036;
	}

	public void setAae036(Date aae036) {
		this.aae036 = aae036;
	}

	public Date getAae043() {
		return aae043;
	}

	public void setAae043(Date aae043) {
		this.aae043 = aae043;
	}

	public String getAae101() {
		return aae101;
	}

	public void setAae101(String aae101) {
		this.aae101 = aae101;
	}

	public Date getAae136() {
		return aae136;
	}

	public void setAae136(Date aae136) {
		this.aae136 = aae136;
	}

	public String getAca111() {
		return aca111;
	}

	public void setAca111(String aca111) {
		this.aca111 = aca111;
	}

	public String getAca112() {
		return aca112;
	}

	public void setAca112(String aca112) {
		this.aca112 = aca112;
	}

	public String getAcb035() {
		return acb035;
	}

	public void setAcb035(String acb035) {
		this.acb035 = acb035;
	}

	public String getAcb200() {
		return acb200;
	}

	public void setAcb200(String acb200) {
		this.acb200 = acb200;
	}

	public String getAcb201() {
		return acb201;
	}

	public void setAcb201(String acb201) {
		this.acb201 = acb201;
	}

	public String getAcb202() {
		return acb202;
	}

	public void setAcb202(String acb202) {
		this.acb202 = acb202;
	}

	public String getAcb203() {
		return acb203;
	}

	public void setAcb203(String acb203) {
		this.acb203 = acb203;
	}

	public String getAcb204() {
		return acb204;
	}

	public void setAcb204(String acb204) {
		this.acb204 = acb204;
	}

	public String getAcb205() {
		return acb205;
	}

	public void setAcb205(String acb205) {
		this.acb205 = acb205;
	}

	public String getAcb206() {
		return acb206;
	}

	public void setAcb206(String acb206) {
		this.acb206 = acb206;
	}

	public Date getAcb207() {
		return acb207;
	}

	public void setAcb207(Date acb207) {
		this.acb207 = acb207;
	}

	public String getAcb208() {
		return acb208;
	}

	public void setAcb208(String acb208) {
		this.acb208 = acb208;
	}

	public BigDecimal getAcb209() {
		return acb209;
	}

	public void setAcb209(BigDecimal acb209) {
		this.acb209 = acb209;
	}

	public String getAcb20b() {
		return acb20b;
	}

	public void setAcb20b(String acb20b) {
		this.acb20b = acb20b;
	}

	public String getAcb20c() {
		return acb20c;
	}

	public void setAcb20c(String acb20c) {
		this.acb20c = acb20c;
	}

	public String getAcb20d() {
		return acb20d;
	}

	public void setAcb20d(String acb20d) {
		this.acb20d = acb20d;
	}

	public Date getAcb20e() {
		return acb20e;
	}

	public void setAcb20e(Date acb20e) {
		this.acb20e = acb20e;
	}

	public String getAcb20f() {
		return acb20f;
	}

	public void setAcb20f(String acb20f) {
		this.acb20f = acb20f;
	}

	public String getAcb210() {
		return acb210;
	}

	public void setAcb210(String acb210) {
		this.acb210 = acb210;
	}

	public Short getAcb211() {
		return acb211;
	}

	public void setAcb211(Short acb211) {
		this.acb211 = acb211;
	}

	public String getAcb214() {
		return acb214;
	}

	public void setAcb214(String acb214) {
		this.acb214 = acb214;
	}

	public String getAcb215() {
		return acb215;
	}

	public void setAcb215(String acb215) {
		this.acb215 = acb215;
	}

	public String getAcb216() {
		return acb216;
	}

	public void setAcb216(String acb216) {
		this.acb216 = acb216;
	}

	public String getAcb217() {
		return acb217;
	}

	public void setAcb217(String acb217) {
		this.acb217 = acb217;
	}

	public Short getAcb218() {
		return acb218;
	}

	public void setAcb218(Short acb218) {
		this.acb218 = acb218;
	}

	public Short getAcb21a() {
		return acb21a;
	}

	public void setAcb21a(Short acb21a) {
		this.acb21a = acb21a;
	}

	public String getAcb21c() {
		return acb21c;
	}

	public void setAcb21c(String acb21c) {
		this.acb21c = acb21c;
	}

	public Short getAcb21d() {
		return acb21d;
	}

	public void setAcb21d(Short acb21d) {
		this.acb21d = acb21d;
	}

	public String getAcb21g() {
		return acb21g;
	}

	public void setAcb21g(String acb21g) {
		this.acb21g = acb21g;
	}

	public BigDecimal getAcb21h() {
		return acb21h;
	}

	public void setAcb21h(BigDecimal acb21h) {
		this.acb21h = acb21h;
	}

	public String getAcb21i() {
		return acb21i;
	}

	public void setAcb21i(String acb21i) {
		this.acb21i = acb21i;
	}

	public Short getAcb221() {
		return acb221;
	}

	public void setAcb221(Short acb221) {
		this.acb221 = acb221;
	}

	public Short getAcb222() {
		return acb222;
	}

	public void setAcb222(Short acb222) {
		this.acb222 = acb222;
	}

	public String getAcb228() {
		return acb228;
	}

	public void setAcb228(String acb228) {
		this.acb228 = acb228;
	}

	public String getBac299() {
		return bac299;
	}

	public void setBac299(String bac299) {
		this.bac299 = bac299;
	}

	public String getBcb991() {
		return bcb991;
	}

	public void setBcb991(String bcb991) {
		this.bcb991 = bcb991;
	}

	public String getAab002() {
		return aab002;
	}

	public void setAab002(String aab002) {
		this.aab002 = aab002;
	}

	public String getAab007() {
		return aab007;
	}

	public void setAab007(String aab007) {
		this.aab007 = aab007;
	}

	public String getAab043() {
		return aab043;
	}

	public void setAab043(String aab043) {
		this.aab043 = aab043;
	}

}