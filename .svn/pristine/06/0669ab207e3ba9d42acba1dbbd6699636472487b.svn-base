<!--/lemis/recommendation/employinvite/AddEmployInvite.jsp-->
<%@ page contentType="text/html; charset=GBK" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.ArrayList" %>
<%@ page import="org.radf.plat.taglib.Button" %>
<%@ page import="org.radf.apps.recommendation.employinvite.form.Rec0401Form" %>
...
Rec0401Form form = (Rec0401Form) request.getSession().getAttribute("Rec0401Form");
LoginDTO userform = (LoginDTO) request.getSession().getAttribute("LoginDTO");
 ...
 if((String)request.getParameter("addE") != null)
  ...
  buttons.add(new Button("Btn_save","保 存","rec040102","save()"));
  buttons.add(new Button("Btn_next","下一步","rec040103","next()"));
...
<script src="/lemis/js/lemisTree.js"></script>
  ...
  <%//录入部分%>
  <html:form action="/Rec0401Action.do?method=addEmployInvite" method="POST">
  ...
  <lemis:editorlayout/>
   <tr>
    ...
    <tr>
      <lemis:codelisteditor type="acb201" label="招聘方式"  isSelect="false" redisplay="true" required="false" key="1"/>
    ...
    <tr>
      <lemis:texteditor property="acb204" label="工作地点" maxlength="80" disable="false" required="true" colspan="3" />
  ...
  function save(){
   ...
   if (!ok){
      return ok;
		...
		obj.action= '<html:rewrite page="/Rec0401Action.do?method=addEmployInvite"/>';
...
function next (obj){
 ...
 var ok = document.all("acb200").value;
    if (ok=="")
   {
    ...
    obj.action= '<html:rewrite page="/Rec0402Action.do?method=toNext&"/>'+"acb200="+ok+"&aab001="+aab001;
