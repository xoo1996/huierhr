<!--/recommendation/statitics/modify.jsp-->
...
<%@ page import="org.radf.apps.commons.entity.Rt98"%>
...
<lemis:base />
	<lemis:body>
		<%
			...
			buttons.add(new Button("query", "查 询", "rec060301",
					"query(document.forms[0])"));
			buttons.add(new Button("resetForm", "重 置", "rec999998",
					"resetForm(document.forms[0])"));

			pageContext.setAttribute("buttons", buttons);

			List buttons1 = new ArrayList();
			buttons1.add(new Button("print", "打　印", "rec060302",
					"printreport()"));
			buttons1.add(new Button("update", "修　正", "rec060303",
					"saveinfo(document.forms[1])"));

			buttons1.add(new Button("Btn_close","关 闭","rec999999","closeWindow(\"Rec06Form\")"));
			pageContext.setAttribute("buttons1", buttons1);

			session.setAttribute("tableheader", "职业介绍工作情况表");//表头
			List listrt98 = (List) session.getAttribute("listrt98");
         ...
         boolean readonly = false;
          if("1".equals(exist)){readonly=true;}
		%>
		...
		<html:form action="/Rec06Action.do?method=query" method="POST">
			...
			<table class="tableinput">
				...
				<lemis:codelisteditor type="year" redisplay="true" dataMeta="data1" label="年度" required="true"/>
					<lemis:texteditor property="ssjqnm" required="true" label="机构"
			...
			if(request.getParameter("flag")!=null){
				if (listrt98 != null) {
					...
					for (int i = 0; i < listrt98.size(); i++) {
						Rt98 rt98 = new Rt98();
						ClassHelper.copyProperties(listrt98.get(i), rt98);
						String qmjggs = rt98.getBrt001();//期末职业机构个数
						String qmjgrs = rt98.getBrt002();//期末职业机构人数
						String bqzprs = rt98.getBrt003();//本期招聘人数
						String bqdjrs = rt98.getBrt004();//本期登记人数
						String bqdjnx = rt98.getBrt005();//本期登记女性
						String bqdjxg = rt98.getBrt006();//下岗
						String bqdjsy = rt98.getBrt007();//失业
						String bqdjzy = rt98.getBrt008();//职业资格
						String bqzjzd = rt98.getBrt009();//本期职业指导人数
						String bqjscg = rt98.getBrt010();//本期介绍成功人数
						String bqjsnx = rt98.getBrt011();//本期介绍成功人数(女性)
						String bqjsxg = rt98.getBrt012();//本期介绍成功人数(下岗)
						String bqjssy = rt98.getBrt013();//本期介绍成功人数(失业)
						String bqjszy = rt98.getBrt014();//本期介绍成功人数(职业资格)
						String brt999 = rt98.getBrt999();//流水号
						String xh = rt98.getBrt015();//序号
						Date sbrq = rt98.getAae036();//上报日期
						String brt996 = rt98.getBrt996();//统计时间
						String cce001 = rt98.getCce001();//填报单位
						String brt048 = rt98.getBrt048();//填报单位
					...
					.convertDateToYearMonthDay((Date) report1.get("sbrq")); //当前时间
					...
					<tr>
						...
						<td>
							<html:text size="3" property="brt001"
								...
								onblur="totalcompute()" readonly="<%=readonly%>"/>
						</td>
						<td>
					...
					<tr>
						<td colspan="16">
							...
							<p align="left">
								...
								<html:text size="3" property="brt001"
									value="<%=report8.get("qmjggs").toString()%>" readonly="<%=readonly%>"/>
								个，财政差额拨款的
								<html:text size="3" property="brt002"
									value="<%=report8.get("qmjgrs").toString()%>" readonly="<%=readonly%>"/>
								个，自收自支的
								<html:text size="3" property="brt003"
									value="<%=report8.get("bqzprs").toString()%>" readonly="<%=readonly%>"/>
								个
							</p>
							<p>
								&nbsp;
...
<%session.setAttribute("listrt98",null); %>
<%session.setAttribute("exist",null); %>
</html>

<script language="javascript">
 var exist;
 if(document.all.exist==undefined){

 }else{
   exist = document.all.exist.value;
 }
...
function printreport()
{
	...
	window.open('/'+lemis.WEB_APP_NAME+'/recommendation/Rec06Action.do?method=printRt98&'+para,'print1','height='+(screen.height-80)+ ',width='+(screen.width-20)+',top=5, left=5, toolbar=no, menubar=yes, scrollbars=yes,resizable=yes,location=no, status=yes');
    ...
    function query(obj){
	 var para = getAlldata(obj);
		  ...
		  obj.action = '<html:rewrite page="/Rec06Action.do?method=viewRt98&"/>'+para;
	      obj.submit();	  
	}
	...
	function saveinfo(obj){
        ...
        var para="cce001="+document.all.cce001.value;
        para+="&brt996="+document.all.year.value;
        obj.action = '<html:rewrite page="/Rec06Action.do?method=saveRt98&"/>'+para;		
		obj.submit();
	}
