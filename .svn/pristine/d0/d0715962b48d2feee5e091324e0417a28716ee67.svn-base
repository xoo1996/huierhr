package org.radf.apps.commons.entity;

import java.util.Date;
import org.radf.plat.util.entity.EntitySupport;

//鉴定机构表$obt
public class Hb41 extends EntitySupport {
    private String aaa021;//行政区划名称

    private Date aab011;//批准日期

    private String aab012;//鉴定机构的批准文号

    private String aab019;//单位类型

    private String aab021;//隶属关系

    private String aab023;//主管部门

    private String aab027;//支付开户银行行号

    private String aab028;//支付开户银行户名

    private String aab029;//支付开户银行账号

    private String aab045;//主办部门

    private String aab301;//所在地行政区划代码

    private String aae005;//联系电话

    private String aae006;//单位地址

    private String aae007;//邮政编码

    private String aae011;//经办人

    private String aae013;//备注

    private String aae016;//网址

    private String aae017;//经办机构

    private String aae019;//经办科室

    private Date aae036;//经办日期

    private String aae100;//通过标记

    private String aae101;//修改人员

    private Date aae136;//修改时间

    private Date ace014;//有效日期

    private String ahb407;//负责人

    private String ahb409;//收费许可证号

    private String ahb40a;//票据准购证

    private String bhc411;//鉴定机构名称

    private String bhc412;//鉴定机构字号

    private String bhc413;//鉴定机构类别

    private Date bhc414;//鉴定机构申请日期

    private String bhc481;//鉴定机构编号

    public String getAaa021() {
        return this.aaa021;
    }

    public void setAaa021(String aaa021) {
        this.aaa021 = aaa021;
    }

    public Date getAab011() {
        return this.aab011;
    }

    public void setAab011(Date aab011) {
        this.aab011 = aab011;
    }

    public String getAab012() {
        return this.aab012;
    }

    public void setAab012(String aab012) {
        this.aab012 = aab012;
    }

    public String getAab019() {
        return this.aab019;
    }

    public void setAab019(String aab019) {
        this.aab019 = aab019;
    }

    public String getAab021() {
        return this.aab021;
    }

    public void setAab021(String aab021) {
        this.aab021 = aab021;
    }

    public String getAab023() {
        return this.aab023;
    }

    public void setAab023(String aab023) {
        this.aab023 = aab023;
    }

    public String getAab027() {
        return this.aab027;
    }

    public void setAab027(String aab027) {
        this.aab027 = aab027;
    }

    public String getAab028() {
        return this.aab028;
    }

    public void setAab028(String aab028) {
        this.aab028 = aab028;
    }

    public String getAab029() {
        return this.aab029;
    }

    public void setAab029(String aab029) {
        this.aab029 = aab029;
    }

    public String getAab045() {
        return this.aab045;
    }

    public void setAab045(String aab045) {
        this.aab045 = aab045;
    }

    public String getAab301() {
        return this.aab301;
    }

    public void setAab301(String aab301) {
        this.aab301 = aab301;
    }

    public String getAae005() {
        return this.aae005;
    }

    public void setAae005(String aae005) {
        this.aae005 = aae005;
    }

    public String getAae006() {
        return this.aae006;
    }

    public void setAae006(String aae006) {
        this.aae006 = aae006;
    }

    public String getAae007() {
        return this.aae007;
    }

    public void setAae007(String aae007) {
        this.aae007 = aae007;
    }

    public String getAae011() {
        return this.aae011;
    }

    public void setAae011(String aae011) {
        this.aae011 = aae011;
    }

    public String getAae013() {
        return this.aae013;
    }

    public void setAae013(String aae013) {
        this.aae013 = aae013;
    }

    public String getAae016() {
        return this.aae016;
    }

    public void setAae016(String aae016) {
        this.aae016 = aae016;
    }

    public String getAae017() {
        return this.aae017;
    }

    public void setAae017(String aae017) {
        this.aae017 = aae017;
    }

    public String getAae019() {
        return this.aae019;
    }

    public void setAae019(String aae019) {
        this.aae019 = aae019;
    }

    public Date getAae036() {
        return this.aae036;
    }

    public void setAae036(Date aae036) {
        this.aae036 = aae036;
    }

    public String getAae100() {
        return this.aae100;
    }

    public void setAae100(String aae100) {
        this.aae100 = aae100;
    }

    public String getAae101() {
        return this.aae101;
    }

    public void setAae101(String aae101) {
        this.aae101 = aae101;
    }

    public Date getAae136() {
        return this.aae136;
    }

    public void setAae136(Date aae136) {
        this.aae136 = aae136;
    }

    public Date getAce014() {
        return this.ace014;
    }

    public void setAce014(Date ace014) {
        this.ace014 = ace014;
    }

    public String getAhb407() {
        return this.ahb407;
    }

    public void setAhb407(String ahb407) {
        this.ahb407 = ahb407;
    }

    public String getAhb409() {
        return this.ahb409;
    }

    public void setAhb409(String ahb409) {
        this.ahb409 = ahb409;
    }

    public String getAhb40a() {
        return this.ahb40a;
    }

    public void setAhb40a(String ahb40a) {
        this.ahb40a = ahb40a;
    }

    public String getBhc411() {
        return this.bhc411;
    }

    public void setBhc411(String bhc411) {
        this.bhc411 = bhc411;
    }

    public String getBhc412() {
        return this.bhc412;
    }

    public void setBhc412(String bhc412) {
        this.bhc412 = bhc412;
    }

    public String getBhc413() {
        return this.bhc413;
    }

    public void setBhc413(String bhc413) {
        this.bhc413 = bhc413;
    }

    public Date getBhc414() {
        return this.bhc414;
    }

    public void setBhc414(Date bhc414) {
        this.bhc414 = bhc414;
    }

    public String getBhc481() {
        return this.bhc481;
    }

    public void setBhc481(String bhc481) {
        this.bhc481 = bhc481;
    }

}
