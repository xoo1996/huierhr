		...
		//审核信息
  		function verifyapa(obj){
			...
			if(confirm("确定要审核通过该离职流程吗？")){
				obj.action = '<html:rewrite page="/ApaOperAction.do?method=verifyApa&type=leave&"/>'+getAlldata(obj);			
				obj.submit();
			}
  		}
  	//回退信息
  		function backapa(obj){
  			...
  			if(confirm("确定要审核通过该离职流程吗？")){
				obj.action = '<html:rewrite page="/ApaOperAction.do?method=backApa&type=leave&"/>'+getAlldata(obj);			
				obj.submit();
  			}
  		}
	</script>
		...
		<table class="tableinput">
			...
			<html:form action="/ApaOperAction.do?" method="POST">
				...
				<tr>
					...
					<lemis:texteditor property="nemadv" label="上级主管意见" required="true" disable="false" colspan="3"/>
				</tr>
				<tr>
					<lemis:formateditor mask="date" property="nemldt" required="false" label="离职时间" disable="false" format="true" />
					<lemis:formateditor mask="date" property="nemedt" required="false" label="工资发放至" disable="false" format="true" />
					<lemis:texteditor property="nemday" label="本月出勤天数" required="true" disable="false" />
				</tr>
				<tr>
					<lemis:texteditor property="nemif" label="其他约定" required="false" disable="false" colspan="3"/>
				</tr>
				</table>
