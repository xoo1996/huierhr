<!--sysmanager/param/AddParameter.jsp-->
...
<%String addurl = "";
			...
			buttons.put("关 闭", "closeWindow(\"aa01Form\")");
...
<lemis:body>
	<lemis:errors />
	<lemis:title title="添加参数信息" />
	<html:form action="/ParameterAction.do?method=addaa01" method="POST">
		...
		<table class="tableInput">
			...
			<tr>
				...
				<lemis:codelisteditor type="aae140" label="参数类型" isSelect="true"
					redisplay="true" required="true" />
					<%if("gs".equals(menuId)){ %>
				<lemis:codelisteditor type="aa001a" label="公式类别" isSelect="true"
					redisplay="true" required="true" />
				<%}else if("lc".equals(menuId)){ %>
				<lemis:codelisteditor type="a001aa" label="流程类别" isSelect="true"
					redisplay="true" required="true" />
				<%}else{ %>
				<lemis:texteditor property="aaa001" label="参数类别代码" required="true"
					disable="false" maxlength="20" />
				<%} %>
					<lemis:texteditor property="aaa003" label="参数代码" required="true"
					disable="false" maxlength="20" />
			</tr>
			<tr>
			<%if(!"lc".equals(menuId)&&!"gs".equals(menuId)){ %>
					<lemis:texteditor property="aaa002" label="参数类别名称" required="false"
					disable="false" maxlength="50" />
					<%} %>
...
<script language="javascript">
   ...
   if (!ok){
     ...
     obj.action= '<html:rewrite page="/ParameterAction.do?method=addaa01&"/>';
     obj.submit();
  }
</script>
