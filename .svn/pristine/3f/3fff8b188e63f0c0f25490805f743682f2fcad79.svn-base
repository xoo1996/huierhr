...
<body onload='print();'>
<p align="center"><OBJECT classid=CLSID:8856F961-340A-11D0-A96B-00C04FD705A2 height=0
...
<table width="905" border="0">
	<tr>
		<td align="center" colspan="2"><span class="title">表6：需求大于求职缺口最大的十个职业（职业小类）</span></td>
	</tr>
	<tr>
...
<table width="905" height="79" border="0" cellpadding="0"
	...
	<%try {
				...
				String sql = "";
				sql = "select tb.*,rownum from (select b.aca111,b.aca112,nvl(b.xqrs,0) as xqrs,nvl(a.qzrs,0) as qzrs,nvl(b.xqrs,0)-nvl(a.qzrs,0) as qk,round(case nvl(a.qzrs,0) when 0 then 0 else nvl(b.xqrs,0)/nvl(a.qzrs,0) end,2) as bl from (select aca111,max(aca112) as aca112,sum(nvl(acb21d,0)) as xqrs from cb21,cb20 where cb21.acb200=cb20.acb200 and aae043<to_date('"
						+ date2
						...
						+ "','yyyy-mm') and aae043>=to_date('"
						+ date1
						+ "','yyyy-mm')  and f_getdata_area('"+a021+"',cc20.aae017,"+sfbh+")='1' group by cc21.aca111)a where a.aca111(+)=b.aca111 and nvl(b.xqrs,0)-nvl(a.qzrs,0)>0 order by nvl(b.xqrs,0)-nvl(a.qzrs,0) desc)tb where rownum<11";
				System.out.println("sql06==" + sql);
