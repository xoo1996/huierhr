...
<%
	...
	header.add(new Formatter("pdtminsto", "商品最小库存量"));
...
//$("input[name=pdtid]").bind("click",function(){
	...
	function add(obj) {
		obj.action = '<html:rewrite href="/huiermis/switchAction.do?prefix=/product&page=/accessory/new.jsp"/>';
		obj.submit();
	};
	function modify(obj) {
		var t = editObj("chk");
		...
		obj.action = '<html:rewrite page="/ProductAction.do?method=modify&mark=accessory&"/>' + getAlldata(obj);
		obj.submit();
	};
	function del(obj) {
		var t = editObj("chk");
...
<lemis:base />
<lemis:body>
	<lemis:title title="配件管理" />
	<lemis:query action="/ProductAction.do?method=query&mark=accessory"
		editorMeta="editor" topic="查询条件" />
	<lemis:table action="/ProductAction.do" headerMeta="header"
		topic="代码信息" hiddenMeta="hidden" mode="radio" />
