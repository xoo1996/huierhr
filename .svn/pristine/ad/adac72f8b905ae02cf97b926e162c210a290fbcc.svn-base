	...
	<script language="javascript">
		...
		//审核信息
  		function verifyapa(obj){
			...
			if(confirm("确定要审核通过该请假流程吗？")){
				obj.action = '<html:rewrite page="/ApaOperAction.do?method=verifyApa&type=rest&"/>'+getAlldata(obj);			
				obj.submit();
			}
  		}
  	//回退信息
  		function backapa(obj){
  			...
  			if(confirm("确定要审核通过该请假流程吗？")){
			obj.action = '<html:rewrite page="/ApaOperAction.do?method=backApa&type=rest&"/>'+getAlldata(obj);			
			obj.submit();
  			}
  		}
	</script>
