...
public class FindTransLogByLoginNameAction extends ActionSupport{
    private String className = this.getClass().getName();

    /**
     * 调用TransFacade.findTransLogByUserId()
     */
	public EventResponse perform(Event evt) {
        ...
        try {
            ...
            // 调用对应的Facade业务处理方法
            ResponseEnvelop resEnv = facade.findTransLogByLoginName(value);
    ...
    protected Object processBody(HashMap hashBody) throws ManageInputException {
		String loginName = TypeCast.stringToString((String) hashBody.get("userId"),"用户id",false);      //用户登录名，不是id
		...
		if (loginName.length() > 12)
			throw new ManageInputException("不合法的用户登录名");

		hashBody.put("loginName", loginName);
