...
<%  
	...
	header.add(new Formatter("fdtsid","耳模编号"));
	...
	editors.add(new Editor("text", "fdtsid", "耳模编号")); 
	...
	hidden.put("fdtsid","耳模编号");
	hidden.put("tmepid","耳膜型号");
...
<script language="javascript">	
	...
	function add() {
		...
		+ "/order/OrderAction.do?method=printClient&tp=earRep&"
	...
	function addClient(obj) {
		...
		window.location.href = "/" + lemis.WEB_APP_NAME
		+ "/order/CustomOrderAction.do?method=forwardClient&tp=earRep&ictnm=" + $("input[name=ictnm]").val();
	}; 
</script>
...
<lemis:body>
	...
	<lemis:query action="/CustomOrderAction.do?method=queryClient&tp=earRep"
