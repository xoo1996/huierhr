package org.radf.apps.commons.entity;

import java.util.Date;
import org.radf.plat.util.entity.EntitySupport;

//定制机信息表
public class Customization extends EntitySupport {
	private String tmkcode; // 定制机条形码
	private String tmkfno; // 订单号
	private String tmkcltid; // 病人代码
	private String tmkplr;// 左右
	private String tmkpid; // 耳机代码
	private String tmkpnl; // 面板编码
	private Double tmkpls; // 增益
	private String tmkmixer; // 配料员代码
	private Date tmkmixdt; // 配料日期
	private String tmksid; // 机身号
	private String tmkshmk; // 外壳制作者代码
	private String tmkshinst; // 外壳安装者代码
	private String tmkshct; // 切磨抛光者代码
	private String tmkshine; //
	private String tmkpol;
	private String tmkmdrt;
	private Date tmkfmdt; // 完工日期
	private String tmknt; // 备注
	private Date tmkpdt; // 计划完工日期
	private String tmkpst; // 定制机状态
	private String tmkurg; // 是否加急
	private String tmkcpy;// 定制厂商
	// 个人客户表
	private String tmkcltnm; // 病人姓名
	// 商品表
	private String tmkpnm; // 耳机名称

	private Date start;
	private Date end;

	// 2012-1-19新增字段
	private String tmkgctnm;// 团体名称
	private String tmkshmknm; // 外壳制作人员姓名
	private String tmkshinstnm; // 外壳安装人员姓名

	// 2012-1-30新增字段
	private Date tmkshmkdt; // 外壳制作日期
	private Date tmkshinstdt; // 外壳安装日期
	private String tmksheltp;

	private Date tmkrependdt;// 保修截止日期

	// 2012-4-12新增字段
	// 库存表
	private String stoid;// 库存编号
	private String stogrcliid;// 团体客户代码
	private String stoproid;// 商品代码
	private String stoprotype;// 商品类型
	private String stoproname;// 商品名称
	private Integer stoamount;// 数量(入库/出库)
	private String stoamountun;// 数量单位
	private Double stoproprice;// 单价
	private Date stodate;// 出入库日期
	private Integer stoactype;// 动作（1、入库-1、出库）
	private String storemark;// 备注
	private String stooprcd;// 操作员代码
	private String stoprodsc;// 商品描述
	private String stodisc;// 是否已报废
	private String stocllar;// 大类
	private String stoclmid;// 中类
	private String stoclsam;// 小类
	private String isrepair;//是否维修订单

	private String aca112;// 统一工种名称
	private String aca111;// 劳服工种编号
	private String folsta;

	private String tmkebjsid;// 耳背机机身编号
	private Date tmkordate;// 订购日期
	// 耳膜质检打印
	private String zjstart;// 开始时间
	private String zjend;// 结束时间

	public String getIsrepair() {
		return isrepair;
	}

	public void setIsrepair(String isrepair) {
		this.isrepair = isrepair;
	}

	public String getZjstart() {
		return zjstart;
	}

	public void setZjstart(String zjstart) {
		this.zjstart = zjstart;
	}

	public String getZjend() {
		return zjend;
	}

	public void setZjend(String zjend) {
		this.zjend = zjend;
	}

	public Date getTmkordate() {
		return tmkordate;
	}

	public void setTmkordate(Date tmkordate) {
		this.tmkordate = tmkordate;
	}

	public String getTmkebjsid() {
		return tmkebjsid;
	}

	public void setTmkebjsid(String tmkebjsid) {
		this.tmkebjsid = tmkebjsid;
	}

	public String getFolsta() {
		return folsta;
	}

	public void setFolsta(String folsta) {
		this.folsta = folsta;
	}

	public String getTmkcode() {
		return tmkcode;
	}

	public void setTmkcode(String tmkcode) {
		this.tmkcode = tmkcode;
	}

	public String getTmkpst() {
		return tmkpst;
	}

	public void setTmkpst(String tmkpst) {
		this.tmkpst = tmkpst;
	}

	public String getTmkurg() {
		return tmkurg;
	}

	public void setTmkurg(String tmkurg) {
		this.tmkurg = tmkurg;
	}

	public String getTmkfno() {
		return tmkfno;
	}

	public void setTmkfno(String tmkfno) {
		this.tmkfno = tmkfno;
	}

	public String getTmkcltid() {
		return tmkcltid;
	}

	public void setTmkcltid(String tmkcltid) {
		this.tmkcltid = tmkcltid;
	}

	public String getTmkpid() {
		return tmkpid;
	}

	public void setTmkpid(String tmkpid) {
		this.tmkpid = tmkpid;
	}

	public String getTmkmixer() {
		return tmkmixer;
	}

	public void setTmkmixer(String tmkmixer) {
		this.tmkmixer = tmkmixer;
	}

	public Date getTmkmixdt() {
		return tmkmixdt;
	}

	public void setTmkmixdt(Date tmkmixdt) {
		this.tmkmixdt = tmkmixdt;
	}

	public String getTmkplr() {
		return tmkplr;
	}

	public void setTmkplr(String tmkplr) {
		this.tmkplr = tmkplr;
	}

	public String getTmkpnl() {
		return tmkpnl;
	}

	public void setTmkpnl(String tmkpnl) {
		this.tmkpnl = tmkpnl;
	}

	public Double getTmkpls() {
		return tmkpls;
	}

	public void setTmkpls(Double tmkpls) {
		this.tmkpls = tmkpls;
	}

	public String getTmksid() {
		return tmksid;
	}

	public void setTmksid(String tmksid) {
		this.tmksid = tmksid;
	}

	public String getTmkshmk() {
		return tmkshmk;
	}

	public void setTmkshmk(String tmkshmk) {
		this.tmkshmk = tmkshmk;
	}

	public String getTmkshinst() {
		return tmkshinst;
	}

	public void setTmkshinst(String tmkshinst) {
		this.tmkshinst = tmkshinst;
	}

	public String getTmkshct() {
		return tmkshct;
	}

	public void setTmkshct(String tmkshct) {
		this.tmkshct = tmkshct;
	}

	public String getTmkshine() {
		return tmkshine;
	}

	public void setTmkshine(String tmkshine) {
		this.tmkshine = tmkshine;
	}

	public String getTmkpol() {
		return tmkpol;
	}

	public void setTmkpol(String tmkpol) {
		this.tmkpol = tmkpol;
	}

	public String getTmkmdrt() {
		return tmkmdrt;
	}

	public void setTmkmdrt(String tmkmdrt) {
		this.tmkmdrt = tmkmdrt;
	}

	public Date getTmkfmdt() {
		return tmkfmdt;
	}

	public void setTmkfmdt(Date tmkfmdt) {
		this.tmkfmdt = tmkfmdt;
	}

	public String getTmknt() {
		return tmknt;
	}

	public void setTmknt(String tmknt) {
		this.tmknt = tmknt;
	}

	public Date getTmkpdt() {
		return tmkpdt;
	}

	public void setTmkpdt(Date tmkpdt) {
		this.tmkpdt = tmkpdt;
	}

	public String getTmkcltnm() {
		return tmkcltnm;
	}

	public void setTmkcltnm(String tmkcltnm) {
		this.tmkcltnm = tmkcltnm;
	}

	public String getTmkcpy() {
		return tmkcpy;
	}

	public void setTmkcpy(String tmkcpy) {
		this.tmkcpy = tmkcpy;
	}

	public Date getStart() {
		return start;
	}

	public void setStart(Date start) {
		this.start = start;
	}

	public Date getEnd() {
		return end;
	}

	public void setEnd(Date end) {
		this.end = end;
	}

	public String getTmkpnm() {
		return tmkpnm;
	}

	public void setTmkpnm(String tmkpnm) {
		this.tmkpnm = tmkpnm;
	}

	public String getTmkshmknm() {
		return tmkshmknm;
	}

	public void setTmkshmknm(String tmkshmknm) {
		this.tmkshmknm = tmkshmknm;
	}

	public String getTmkshinstnm() {
		return tmkshinstnm;
	}

	public void setTmkshinstnm(String tmkshinstnm) {
		this.tmkshinstnm = tmkshinstnm;
	}

	public String getTmkgctnm() {
		return tmkgctnm;
	}

	public void setTmkgctnm(String tmkgctnm) {
		this.tmkgctnm = tmkgctnm;
	}

	public Date getTmkshmkdt() {
		return tmkshmkdt;
	}

	public void setTmkshmkdt(Date tmkshmkdt) {
		this.tmkshmkdt = tmkshmkdt;
	}

	public Date getTmkshinstdt() {
		return tmkshinstdt;
	}

	public void setTmkshinstdt(Date tmkshinstdt) {
		this.tmkshinstdt = tmkshinstdt;
	}

	public String getStoproid() {
		return stoproid;
	}

	public void setStoproid(String stoproid) {
		this.stoproid = stoproid;
	}

	public String getStoproname() {
		return stoproname;
	}

	public void setStoproname(String stoproname) {
		this.stoproname = stoproname;
	}

	public Date getStodate() {
		return stodate;
	}

	public void setStodate(Date stodate) {
		this.stodate = stodate;
	}

	public String getStooprcd() {
		return stooprcd;
	}

	public void setStooprcd(String stooprcd) {
		this.stooprcd = stooprcd;
	}

	public String getTmksheltp() {
		return tmksheltp;
	}

	public void setTmksheltp(String tmksheltp) {
		this.tmksheltp = tmksheltp;
	}

	public String getStoid() {
		return stoid;
	}

	public void setStoid(String stoid) {
		this.stoid = stoid;
	}

	public String getStogrcliid() {
		return stogrcliid;
	}

	public void setStogrcliid(String stogrcliid) {
		this.stogrcliid = stogrcliid;
	}

	public String getStoprotype() {
		return stoprotype;
	}

	public void setStoprotype(String stoprotype) {
		this.stoprotype = stoprotype;
	}

	public Integer getStoamount() {
		return stoamount;
	}

	public void setStoamount(Integer stoamount) {
		this.stoamount = stoamount;
	}

	public String getStoamountun() {
		return stoamountun;
	}

	public void setStoamountun(String stoamountun) {
		this.stoamountun = stoamountun;
	}

	public Double getStoproprice() {
		return stoproprice;
	}

	public void setStoproprice(Double stoproprice) {
		this.stoproprice = stoproprice;
	}

	public Integer getStoactype() {
		return stoactype;
	}

	public void setStoactype(Integer stoactype) {
		this.stoactype = stoactype;
	}

	public String getStoremark() {
		return storemark;
	}

	public void setStoremark(String storemark) {
		this.storemark = storemark;
	}

	public String getStodisc() {
		return stodisc;
	}

	public void setStodisc(String stodisc) {
		this.stodisc = stodisc;
	}

	public String getAca112() {
		return aca112;
	}

	public void setAca112(String aca112) {
		this.aca112 = aca112;
	}

	public String getAca111() {
		return aca111;
	}

	public void setAca111(String aca111) {
		this.aca111 = aca111;
	}

	public String getStocllar() {
		return stocllar;
	}

	public void setStocllar(String stocllar) {
		this.stocllar = stocllar;
	}

	public String getStoclmid() {
		return stoclmid;
	}

	public void setStoclmid(String stoclmid) {
		this.stoclmid = stoclmid;
	}

	public String getStoclsam() {
		return stoclsam;
	}

	public void setStoclsam(String stoclsam) {
		this.stoclsam = stoclsam;
	}

	public String getStoprodsc() {
		return stoprodsc;
	}

	public void setStoprodsc(String stoprodsc) {
		this.stoprodsc = stoprodsc;
	}

	public Date getTmkrependdt() {
		return tmkrependdt;
	}

	public void setTmkrependdt(Date tmkrependdt) {
		this.tmkrependdt = tmkrependdt;
	}

}
