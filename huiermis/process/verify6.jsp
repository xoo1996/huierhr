...
<script type="text/javascript">
	...
	//审核信息
		...
		if(confirm("确定要审核通过该入职流程吗？")){
			obj.action = '<html:rewrite page="/ApaOperAction.do?method=verifyApa&type=user&"/>'+getAlldata(obj);			
			obj.submit();
		}
		}
	//回退信息
		function backapa(obj){
			...
			if(confirm("确定回退该入职流程吗？")){
		obj.action = '<html:rewrite page="/ApaOperAction.do?method=backApa&type=user&"/>'+getAlldata(obj);			
		obj.submit();
			}
		}
</script>
		...
		<html:form action="/ApaOperAction.do" method="POST">
			...
			<tr>
				<lemis:texteditor property="bsc009" label="所属部门" disable="true" 
					required="true" />
				<lemis:codelisteditor type="positionname" label="所属岗位" required="false" />
			...
			<tr>
					...
					<lemis:formateditor mask="date" property="userworktime" format="true"
						label="参加工作时间"  required="false" disable="true"/>
			...
			<table class="tableinput">
				...
				<tr>
					...
					<lemis:texteditor property="userpostcode1" label="邮编"
						disable="true" required="false" maxlength="6"/>
				<lemis:codelisteditor type="usercontype" label="合同类型" required="false" isSelect="false" redisplay="true"/>
				</tr>			
			</table>
			...
			<COLGROUP><COL width='20%'><COL width='15%'><COL width='20%'><COL width='15%'><COL width='20%'><COL width='15%'></COLGROUP>
				<tr>
					...
					<lemis:codelisteditor type="investigate3" label="是否与其他单位有未尽法律事宜"
						required="false" />
				</tr>
				<tr>
