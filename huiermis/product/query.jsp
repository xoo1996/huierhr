...
<%
	...
	header.add(new Formatter("pdtlmt","保修期限(月)"));
...
<%-- $(document).ready(function(){
	...
	$("input[name=pdtid]").result(function(event, data, formatted) {
		if (data){
			var pnm = data[0].substring(data[0].indexOf("=")+1,data[0].indexOf(":")).replace(/(^\s*)|(\s*$)/g, "");
...
//});
	...
	function add(obj) {
		obj.action = '<html:rewrite href="/huiermis/switchAction.do?prefix=/product&page=/new.jsp"/>';
		obj.submit();
	};
	function modify(obj) {
		var t = editObj("chk");
...
<lemis:base />
<lemis:body>
	<lemis:title title="商品代码管理" />
	<lemis:query action="/ProductAction.do?method=query&mark=make"
