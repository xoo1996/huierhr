...
<%
	...
	header.add(new Formatter("tmkcltnm", "病人姓名"));
	...
	buttons.put("配料确认","batchConfirm(document.all.tableform)");
	buttons.put("配料有误","batchRefuse(document.all.tableform)");
...
<script language="javascript">
	...
	function batchConfirm() {
		...
		if (confirm("确实要确认选中的配料项吗？")) {
			window.location.href = "/"
					+ lemis.WEB_APP_NAME
					+ "/customization/CustomizationAction.do?method=batchChange&"
					+ getAlldata(document.all.tableform) + "require=confirm";
		}
	...
	function batchRefuse() {
		...
		if (confirm("确实要回退选中的配料项吗？")) {
			window.location.href = "/"
					+ lemis.WEB_APP_NAME
					+ "/customization/CustomizationAction.do?method=batchChange&"
					+ getAlldata(document.all.tableform) + "require=refuse";
		}
		//将数据提交到后台 注意：getAlldata(document.all.tableform)是得到所有要提交的数据。
		else
			return t;
...
<lemis:body>
	...
	<lemis:query
		action="/CustomizationAction.do?method=query&order=confirm"
