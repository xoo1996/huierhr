...
<%
	...
	buttons.put("批量质检","batchQA(document.all.tableform)");
	buttons.put("返 回","history.back()");
...
<script language="javascript">
	$(document).ready(function(){
		...
		for(var i=1; i <= 10 ; i++){
			...
			if($('#qachka_row' + i).val() == '')//如果下拉列表没有值则将默认值设置为yes
			{
				$('#qachka_row' + i).val("yes");
			}
			if($('#tsksta_hrow' + i).val() != '3')//如果面板状态不为等待质检、则将面板文本框设置成readonly
			{
				$('#tsdpnlid_row' + i).attr('readonly','readonly');
			}
		}
	...
	function batchQA(obj){
		...
		obj.action = '<html:rewrite page="/TaskAction.do?method=batchQA"/>';
		obj.submit();
	};
</script>
