...
<%
	...
	header.add(new Formatter("tsknt","备注"));
	header.add(new Formatter("tskdmd", "面板要求", true));

	

	Map<String,String> buttons=new LinkedHashMap<String,String>();
	buttons.put("增 加","add(document.all.tableform)");
	buttons.put("修 改","modify(document.all.tableform)");
	buttons.put("删 除","del(document.all.tableform)");
	buttons.put("面板零件详情","detail(document.all.tableform)");
	buttons.put("面板明细","panel()");
	buttons.put("打印任务单", "print()");
	...
	editors.add(new Editor("text", "tsksta", "面板状态"));
...
<script language="javascript">
  		...
  		function add(obj) {
  			obj.action = '<html:rewrite href="/huiermis/switchAction.do?prefix=/task&page=/taskadd.jsp"/>';
  			obj.submit();
  		};
  		function modify() {
  			var t = editObj("chk");
			if(!t){
  			...
  			window.location.href = "/" + lemis.WEB_APP_NAME
  					+ "/task/TaskAction.do?method=show&"
			...
			if(!t){
  			...
  			if(!confirm("确定删除此任务单？")){
  				window.event.returnValue=false;
  	  			...
  	  			window.location.href = "/" + lemis.WEB_APP_NAME
					+ "/task/TaskAction.do?method=delete&"
					+ getAlldata(document.all.tableform);
  		...
  		function detail() {
			...
			if(!t){
  			...
  			window.location.href = "/" + lemis.WEB_APP_NAME
  					+ "/task/TaskAction.do?method=detail&"
  					+ getAlldata(document.all.tableform);
  		};
  		...
  		function panel(){
  			...
  			if(!t){
  				...
  				window.location.href = "/" + lemis.WEB_APP_NAME
					+ "/task/TaskAction.do?method=quality&tp=check&"
					+ getAlldata(document.all.tableform);
  		};
  		...
  		function print(){
  			...
  			window.location.href = "/" + lemis.WEB_APP_NAME
  				+ "/task/TaskAction.do?method=print&"
  				+ getAlldata(document.all.tableform);
  		};
	</script>
...
<lemis:base />
<lemis:body>
	<lemis:title title="任务单查询" />
	<lemis:query action="/TaskAction.do?method=query&order=task" editorMeta="editor"
		topic="查询条件" />
