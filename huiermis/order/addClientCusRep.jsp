...
<!-- <script src="/huiermis/js/inputDetail.js"> -->
 ...
 $(document).ready(function(){
	...
	function creatOrderh(obj) {
		...
		{
			 var rsheltp=$('input[name=pdtsheltp]').val();
		}
		else
		{
			var lsheltp=$('input[name=pdtsheltp]').val();
		}
		...
		if (lpid == '' && rpid == '') {
			...
			obj.action = '<html:rewrite href="/huiermis/order/CustomOrderAction.do?method=add&type=h&"/>'
					+ 'num=1'
					+ '&cid='
					+ cid
					+ '&plr=l&pid='
					+ lpid+"&lsheltp="+lsheltp
					+ '&cnm=' + cnm;
		} else if (lpid == '') {
			obj.action = '<html:rewrite href="/huiermis/order/CustomOrderAction.do?method=add&type=h&"/>'
					+ 'num=1'
			...
			obj.action = '<html:rewrite href="/huiermis/order/CustomOrderAction.do?method=add&type=h&"/>'
					...
					+ '&lpid='
					+ lpid
					+ '&rpid=' + rpid+"&lsheltp="+lsheltp+"&rsheltp="+rsheltp
					+ '&cnm=' + cnm;
		}
...
function lgvag() {
	...
	function xiugai(obj){
		...
		{  
			...
			{
				obj.action = '<html:rewrite page="/CustomOrderAction.do?method=addClient&tp=cusRep&type=firm&"/>' + getAlldata(obj);
			    obj.submit();
			}
		}
	};
...
<lemis:body>
	...
	<table class="tableInput">
		<lemis:editorlayout />
			<html:form action="/CustomOrderAction.do?method=addClient&tp=cusRep&type=o" method="POST">
			<tr>
