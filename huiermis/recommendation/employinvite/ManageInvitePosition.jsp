...
<%@ page import="org.radf.apps.recommendation.employinvite.dto.Rec0402DTO" %>
...
LoginDTO userform = (LoginDTO) request.getSession()
 ...
 url = (String)request.getSession().getAttribute("nextP");
  ...
  dto = (Rec0402DTO)request.getSession().getAttribute("Rec0402DTO");
  ...
  List buttons = new ArrayList();
    ...
    buttons.add(new Button("Btn_view", "查 看","rec040205", "view(document.all.tableform)"));
  	buttons.add(new Button("Btn_ad", "增 加","rec040206", "add(document.all.tableform)"));
  	buttons.add(new Button("Btn_edit", "修 改","rec040207", "edit(document.all.tableform)"));
  	buttons.add(new Button("Btn_del", "停止招工","rec040208", "stop()"));
	buttons.add(new Button("Btn_delete", "删除招工","rec040211", "del()"));
  buttons.add(new Button("Btn_back", "返 回","rec999997", "go2Page(\"recommendation\",\"1\")"));
  buttons.add(new Button("Btn_close", "关 闭","rec999999", "closeWindow(\"Rec0402DTO,Rec0402Form\")"));
...
%>
  ...
  <%//显示批量查询结果信息%>
  <lemis:table topic="空岗信息" action="/Rec0402Action.do" headerMeta="header" hiddenMeta="hidden" mode="radio" appendBlank="false"/>
...
<script language="javascript" >
  ...
  // 新增招聘空位信息
  function add(obj){
    obj.action= '<html:rewrite page="/Rec0402Action.do?method=addEmptyPost&"/>'+getAlldata(document.all.tableform);
    obj.submit();
  }
  ...
  function edit(obj){
    ...
    obj.action= '<html:rewrite page="/Rec0402Action.do?method=modEmptyPost&"/>'+getAlldata(document.all.tableform);   
    obj.submit();
  }
  // 删除空位信息
  function stop(){
    ...
    if(confirm("此操作不能回退，确信要停止招工吗？")){
      var url = '<html:rewrite page="/Rec0402Action.do?method=delEmptyPost&"/>'+getAlldata(document.all.tableform);
  ...
  function del(){
    ...
    if(confirm("此操作不能回退，确信要删除您选中的信息吗？")){
      var url = '<html:rewrite page="/Rec0402Action.do?method=deleteEmptyPost&"/>'+getAlldata(document.all.tableform);
  ...
  // 查看招聘空位信息
   function view(obj){
    ...
    obj.action= '<html:rewrite page="/Rec0402Action.do?method=viewEmptyPost&"/>'+getAlldata(document.all.tableform);
