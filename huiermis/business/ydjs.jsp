...
<script language="javascript">
	...
	function startRequest(obj) {
		$.getJSON(
						"/huiermis/business/BusinessAction.do?method=preyuejie&time="
								+ new Date(),
						createQueryString(),
						function(data) {
							if (data[0].flag == '1') {
								if (confirm("该月已完成结算，要重做吗？此操作无法回退！")) {
									if (confirm("确定要重做吗？此操作无法回退！")) {
										alert("正在执行 "+$('input[name=ivtyear]').val() + "年" + $('input[name=ivtmonth]').val()+"月的月结,请耐心等待...");
										obj.action = '<html:rewrite page="/BusinessAction.do?method=yuejie"/>' + getAlldata(obj);
										obj.submit();
									}
								}
							} else {
