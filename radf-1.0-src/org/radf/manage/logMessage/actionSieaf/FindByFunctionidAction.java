...
/**
 * <p>Title: 根据交易号进行日志查询</p>
 * <p>Description:Action</p>
 * @author zqb
 * @version 1.0
 */

public class FindByFunctionidAction extends ActionSupport {
    private String className = this.getClass().getName();

    /**
     * 调用LogFacade.findByFunctionid
     * @param evt
     * @return
     */
    public EventResponse perform(Event evt) {
        ...
        try {
            ...
            // 调用对应的Facade业务处理方法
            ResponseEnvelop resEnv = facade.findByFunctionid(value);
    ...
    protected Object processBody(HashMap hashBody) throws ManageInputException {
        // check param
        String functionid = TypeCast.stringToString((String) hashBody
                .get("functionid"), "日志功能号", false);

        // 分页标记
        String count = TypeCast.intToString(TypeCast.stringToInt(
                (String) hashBody.get("count"), "个数", false));
        String offset = TypeCast.intToString(TypeCast.stringToInt(
                (String) hashBody.get("offset"), "偏移量", false));

        HashMap reqbody = new HashMap();
        reqbody.put("functionid", functionid);
