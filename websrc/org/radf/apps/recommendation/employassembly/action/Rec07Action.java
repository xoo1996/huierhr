...
import org.radf.apps.recommendation.employassembly.form.Rec07Form;
 ...
 * 大会招聘信息管理
 */
public class Rec07Action extends ActionLeafSupport {
	...
	public ActionForward findPer(ActionMapping mapping, ActionForm form,
		...
		Rec07Form fm = (Rec07Form) req.getSession().getAttribute("Rec07Form");
	...
	public ActionForward queryEmployer(ActionMapping mapping,
		...
		Rec07Form form = (Rec07Form) actionForm;
		String forward = "queryEmployer";
		...
		ab01.setFileKey("rec07_001");
		// 调用接口进行查询
		try {
			String hql = queryEnterprise(ab01);
			af = super.init(request,
					"/recommendation/employassembly/QueryEmployer.jsp", hql,"1");
			Collection data = (Collection) request
					.getAttribute(GlobalNames.QUERY_DATA);
			if (data == null) {
				request.getSession().setAttribute("Rec07Form", form);
				forward = "check";
			}
		} catch (Exception e) {
